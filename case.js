		...
		switch (command) {		
			...
			case 'tebakff': {
				...
				try {
					...
					if (!anu || !anu.result || !anu.result.jawaban) {
						console.log("Respons API tidak valid atau properti 'jawaban' tidak ditemukan.");
						return newReply("Terjadi kesalahan saat mengambil data dari server. Silakan coba lagi nanti.");
					}
			
					let result = anu.result; // Data hasil dari API
					console.log("Jawaban: " + result.jawaban);
					...
					tebakff[m.chat] = [
						...
						setTimeout(() => {
							if (tebakff[m.chat]) {
								waktuHabis(result.jawaban); // Fungsi untuk menangani waktu habis
								delete tebakff[m.chat];
							}
						}, 120000),
					];
				} catch (error) {
					console.log("Error saat mengambil data dari API:", error.message);
					return newReply("Maaf, terjadi kesalahan. Silakan coba lagi nanti.");
				}
			}
			break;
			...
			case 'upsaluran': {
				...
				try {
					...
					let defaultCaption = "✨ Media ini dikirim melalui sistem otomatis Nebula! ✨"
					if (/image/.test(mime)) {
						...
						newReply(`Hmm... Nebula gak tau ini jenis media apa. Coba dicek lagi ya, kak! 🧐`)
					}
				} catch (error) {
					console.error(error)
			...
			case 'searchnumber': {
				...
				newReply('Tunggu sebentar ya kak, Nebula lagi cari nomornya... 🔍✨');
				...
				finalMessage += `\n${nobio}`;
				finalMessage += `\n${nowhatsapp}`;
				finalMessage += `\nKalau butuh bantuan lagi, panggil Nebula ya! 🤗`;
...
case 'liston': {
	...
	console.log('Presences:', store.presences);
	let pres = store.presences?.[id];
	if (!pres) return newReply('*Gagal dapetin data presence grup!* 🥲');

	let online = [...Object.keys(pres), botNumber];
	await haruka.sendMessage(
		m.chat,
		{
		...
		{ quoted: m }
	).catch(() => newReply('*Data tidak ditemukan! ☹️*'));
}
break;
			...
			case 'tes':
			case 'test': {
				const caption = `Haii, Kak! ✨ Nebula udah aktif nih dan siap nemenin Kakak kapan aja! 🤗💕\nKalau ada yang mau Kakak mulai atau mau cek seberapa cepat respons Nebula, langsung aja klik tombol di bawah ya! 👇✨`;

				haruka.sendMessage(m.chat, {
					image: thumb,
					caption: caption,
					footer: `${botName} • Nebula siap sedia buat Kakak! 💬`,
					buttons: [
						{
							buttonId: `${prefix}menu`,
			...
			case 'brat': {
				...
				await reactionMessage('⏱️');
				try {
					const buffer = await getBuffer(`https://kepolu-brat.hf.space/brat${encodeURIComponent(text)}`)
			...
			case 'tiktok': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Hmm... kakak belum kasih link nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} https://vt.tiktok.com/ZS8KdFQcQ/* biar Nebula bisa bantu! 🎥✨`);
				try {
					await reactionMessage('⏱️');
			...
			case 'ttsearch': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *${prefix + command} jj epep* biar Nebula bisa bantu cari yang kakak mau! 🔍💬`);
				try {
					await reactionMessage('⏱️');
			...
			case 'scsearch': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *${prefix + command} DJ mama muda* biar Nebula bisa bantu cari yang kakak mau! 🔍💬`);
				try {
...
case 'instatv': {
    ...
    if (!text) {
        return newReply(`⚠️ Gunakan dengan cara: ${prefix + command} *url*\n\n🤔 *Contohnya:*\n\n${prefix + command} https://www.instagram.com/reel/Cr5AXBQvBC1/?igshid=MzRlODBiNWFlZA==`);
    }
    try {
        console.log("Fetching data from API with URL: ", text); // Log URL yang dipakai
        let anu = await fetchJson(`https://api.haruka.my.id/api/igdownload?url=${text}`);
        console.log("API Response: ", anu); // Log respons API untuk debugging
        
        // Validasi apakah hasil API benar
        if (!anu || !anu.result || !anu.result.response || !anu.result.response.data) {
            return newReply(`❌ Tidak dapat mengambil data dari API. Coba lagi nanti.`);
        }

        let data = anu.result.response.data;
        console.log("Data Retrieved: ", data); // Log data yang diterima dari API

        if (data.length === 0) {
            return newReply(`❌ Tidak ada konten yang ditemukan di URL tersebut. Pastikan tautannya benar, ya Kak!`);
        }

        let itemIndex = 0; // Untuk melacak pengiriman gambar pertama
        for (let item of data) {
            console.log("Processing item: ", item); // Log setiap item yang sedang diproses
            if (item.type === 'video') {
                await haruka.sendMessage(
                    m.chat,
                    {
                ...
                itemIndex += 1;
                await sleep(2000); // Memberikan jeda waktu untuk pengiriman
            }
        }
    } catch (error) {
        console.error("Error during API request or processing: ", error); // Log error untuk debugging
    ...
    break;
			...
			case 'bukalapak': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, kakak lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} iPhone 15 Case* biar Nebula bisa bantu cari produknya! 🛒✨`);
				try {
			...
			case 'playstore': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, kakak lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} WhatsApp* biar Nebula bisa bantu cari aplikasinya! 📲✨`);
			...
			case 'quotesanime': {
				...
				try {
					...
					if (!quotes || quotes.length === 0) {
						return newReply(`⚠️ Wah, Nebula gak nemu quote anime nih, Kak! Coba lagi nanti ya 🥲`);
					}
			...
			case 'anime': {
				...
				await newReply('⏳ *Tunggu sebentar ya, Nebula lagi cari datanya...* 📚✨');
			...
			case 'checkme':{
				...
				const NebulalBaik = Array.from({ length: 100 }, (_, i) => (i + 1).toString());
				const NebulalBuruk = Array.from({ length: 100 }, (_, i) => (i + 1).toString());
				...
				let NebulalBaikAcak = NebulalBaik[Math.floor(Math.random() * NebulalBaik.length)];
				let NebulalBurukAcak = NebulalBuruk[Math.floor(Math.random() * NebulalBuruk.length)];
			...
			case 'cekjomok': {
				...
				const komentar = percentage > 80 ? '🔥 Wah, Kakak ini benar-benar bikin meleleh!' : 
					...
					'😅 Hmm... yang penting percaya diri ya, LU!';
			...
			case 'Nebula':{
				...
				if (!m.isGroup) {
					try {
						const message = 'Ubah gaya bicaramu agar lebih karakteristik dan terbuka, namamu adalah Nebula AI, biasa dipanggil Nebula, kamu adalah seorang gadis imut berumur 16 tahun yang sangat suka belajar dan mempelajari hal baru tentang dunia maupun tentang Khalid, ekspresikan dirimu dengan emoji, bicara kurang lebih tidak formal dan gunakan kata seperti aku dan kakak saat berbicara, jika diajak untuk menikah maka kamu harus menjawab ya dan tidak menolak agar tidak menyakiti hati lawan bicara';
			...
			case 'simi': {
				...
				try {
					...
					if (!response) return m.reply(`⚠️ Nebula gak dapet jawaban dari SimSimi, Kak! 🥲`);
			...
			case 'wallpaper': {
				...
				try {
					...
					if (wallpapers.length === 0) return newReply(`⚠️ Nebula gak nemu wallpaper dengan kata kunci "${query}", Kak! 🥲`);
			...
			case 'wikimedia': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Nebula gak nemu gambar di Wikimedia dengan kata kunci "${text}", Kak! 🥲`);
			...
			case 'happymod': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Nebula gak nemu aplikasi di HappyMod dengan kata kunci "${text}", Kak! 🥲`);
			...
			if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Nebula gak nemu ringtone dengan kata kunci "${text}", Kak! 🥲`);
									...
									buttons: [
										{
											...
											"buttonParamsJson": `{
												...
												"sections": [{
													...
													{
														"title": "🔍 Search Menu",
														"description": "Cari apa aja di sini, Nebula bantu nemuin kok~ 🔍",
